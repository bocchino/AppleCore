...
\item \ithead{Call expressions.} (1) The called expression (i.e., the
  ...
  otherwise the size is 0 and the signedness is unsigned.
%
...
%
\item \ithead{Return statements.} (1) If a function has nonzero return
  size, and the function has a body, then (a) the body must end with a
  return statement, and (b) every return statement appearing in the
  body must contain an expression of nonzero size.  (2) If a function
  has return size 0, then no return statement in the function body may
  contain an expression of nonzero size.
%
\end{itemize}
