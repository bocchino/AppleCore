...
are impossible to say in Applesoft or other languages.  For example,
in Applesoft you could do this with the \kwd{USR} function or a
...
\end{enumerate}
%
A use appearing at global scope must come after the corresponding
definition in the source file.  This rule precludes circular
...
\noindent
is not allowed, since the identifier \kwd{B} is not defined at the
point where it is first used.  A use appearing inside a function body
may come before or after the corresponding definition in the source
file.
...
\end{enumerate}
%
This is so that the compiler can precompute the value before
generating any code (in the case of a constant-value expression) or
generate an assembler label (in the other two cases).  In case 3, the
actual address associated with the assembler label may not be known
until final assembly (Section~\ref{sec:code-gen:source-files}).
