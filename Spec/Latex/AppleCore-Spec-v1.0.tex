...
(i.e., $2^{255 \cdot 8}$).  It is a compile-time error to write an
integer constant with a value larger than that.
...
source file not as part of a definition.
...
\item \ithead{Call expressions.} (1) The called expression (i.e., the
  expression before the first parenthesis) must either be an
  identifier corresponding to a function definition or evaluate to a
  1- or 2-byte value.  (2) If the called expression corresponds to a
  function definition, then the size and signedness are the return
  size and signedness of the function definition (see below);
  otherwise the size is 0 and the signedness is unsigned.  (3) Each
  argument expression must have nonzero size.
%
\item \ithead{Set expressions.} The size and signedness of a set
  ...
  expression must have a nonzero size.
%
...
\bfhead{Declarations and statements.} The following rules govern the
size and signedness of declarations and statements:
%
\begin{itemize}
%
\item \ithead{Function return size and signedness.} The \emph{return
  size and signedness} of a function definition are defined as
  follows.  If a \nonterm{size} appears after the keyword \kwd{FN} in
  the definition, then that is the size; otherwise the size is 0.  If
  \kwd{S} appears after \nonterm{size} then the expression is signed,
  otherwise unsigned.  
%
\item \ithead{Function frame size.} The \emph{frame size} of a
  function with a body equals 2 (for the frame pointer) plus the sizes
  of all the function parameters and local variables.  No function may
  have a frame size of greater than 255 bytes.
%
\item \ithead{Return statements.} (1) No return statement may contain
  an expression of zero size.  (2) If a function has nonzero return
  size, and the function has a body, then (a) the body must end with a
  return statement containing an expression, and (b) every return
  statement appearing in the body must contain an expression.  (3) If
  a function has return size 0, then no return statement in the
  function body may contain an expression.
%
\end{itemize}
