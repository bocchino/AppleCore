...
\item \ithead{Unary operation expressions.} The size and signedness of
  ...
  operation is signed; and (3) if negating a constant would cause the
  value to overflow, then one byte is added to the size.  For example,
  the size of the expression \kwd{-200} is 2, because the unsigned
  value 200 can be represented using one byte, but the signed value
  $-200$ cannot.
%
