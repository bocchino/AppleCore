...
%
\item Issue a \kwd{JMP} instruction to the first function with a body
  that apepars in the file.  There must be at least one such function,
...
Global declarations are translated as follows, in the order in which
...
numeric constant, in little-endian order.

\bfhead{Function declarations.} (1) A function declaration is
\emph{external} if it has no body.  An external function declaration
does not correspond to any generated code; its signature is used only
for checking call expressions that invoke the function.  (2) A
