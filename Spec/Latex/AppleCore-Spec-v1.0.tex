...
\bfhead{Computing size and signedness.} Every expression appearing in
the source file is given a \emph{size} and a \emph{signedness},
computed as follows:
%
\begin{itemize}
%
\item \ithead{Identifiers.} The size and signedness of an identifier
  ...
  declaration, and the signedness is signed if \kwd{S} appears after
...
\item \ithead{Binary operation expressions.}  For comparison
  ...
  unless both operands are constant-value expressions
...
\item \ithead{Unary operation expressions.} The size and signedness of
  ...
  an address operation \kwd{@} is two bytes unsigned; (2) a negation
  operation is signed; and (3) if the operand is a constant-value
...
\bfhead{Size and signedness requirements.} The following requirements
apply to the size and signedness of expressions:
%
\begin{itemize}
%
...
\subsubsection{Function Frame Sizes} 

The \emph{frame size} of a function declaration that contains a body
equals 2 (for the frame pointer) plus the sizes of all the function
parameters and local variables.  No function may have a frame size of
greater than 255 bytes.

\subsubsection{Return Statements} 

(1) No return statement may contain an expression of zero size.  (2)
If a function has nonzero return size, and the function has a body,
then (a) the body must end with a return statement containing an
expression, and (b) every return statement appearing in the body must
contain an expression.  (3) If a function has return size 0, then no
return statement in the function body may contain an expression.
...
\bfhead{Definition of constant-value expression.} A constant-value
...
%
\item A binary operation expression, where each operand is a numeric
  constant or an identifier corresponding to a numeric constant
  declaration.
%
\item A negation (\kwd{-}) or \kwd{NOT} unary operation expression,
  where the operand is a numeric constant or an identifier
  corresponding to a numeric constant declaration.
%
...
\bfhead{Requirement of constant-value expressions at global scope.} An
expression appearing at global scope (i.e., in a constant or data
declaration, or as the initializer of a global variable declaration)
must be a constant-value expression.  This is so the compiler can
