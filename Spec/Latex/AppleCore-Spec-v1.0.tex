...
(2) For each expression \nonterm{expr} appearing at global scope, for
each identifier corresponding to a constant declaration that appears
in \nonterm{expr}, the corresponding constant declaration must (a)
precede \nonterm{expr} in the source file or (b) contain no
...
\end{verbatim}
%
is not allowed, since the first use of the identifier \kwd{B} precedes
its definition, and the expression in the definition contains an
identifier (\kwd{A}) corresponding to a constant declaration.  On the
other hand,
%
\begin{verbatim}
CONST A B;
