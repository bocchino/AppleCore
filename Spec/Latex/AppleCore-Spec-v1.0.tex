...
\ithead{Negation (\kwd{-}) and bitwise not (\kwd{NOT}).} ...

\ithead{Increment (\kwd{INCR}) and decrement (\kwd{DECR}).} ...
...
\subsection{Computation of Address for Semantic LValues}
...
\item If \nonterm{expr} is an identifier corresponding to a global
  ...
  (Section~\ref{sec:code-gen:global-decls}). 
%
\item If \nonterm{expr} is an identifier coresponding to a function
...
\item If \nonterm{expr} is a register expression, then its address is
  the lowest address of the function call stack slot assigned to that
