...
that may appear on the left-hand side of a set expression at parse
time.  It is an identifier, an indexed expression, a register
...
(Section~\ref{sec:syntax:expressions}) other than a parentheses
expression and (2) if it is an identifier, then it corresponds to a
...
values saved in their slots on the stack, \kwd{JSR} to the constant
...
value and do a \kwd{JSR} to an indirect \kwd{JMP} to the address.
