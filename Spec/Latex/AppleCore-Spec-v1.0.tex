...
Applesoft you could do this with the \kwd{USR} function or a
\kwd{CALL}, but in either case you'd need to write some glue code in
...
\emph{signedness}.  A constant-value expression
(Section~\ref{sec:semantics:constant-value-expr}) is signed, and its
...
have the maximum allowed size of 255.  For an expression other than a
constant-value expression, size and signedness are computed as
follows:
%
\begin{itemize}
%
...
the order in which they appear in the source file, and either (1)
compute the constant value of \nonterm{expr} (in case 1) or generate
an assembler label for \nonterm{expr} (in cases 2 and 3). In case 3,
the actual address associated with the assembler label may not be
known until final assembly (Section~\ref{sec:code-gen:source-files}).
