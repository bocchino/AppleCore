...
expression, or a numeric constant.
%
$$\nonterm{expr} ::= \mbox{\nonterm{lvalue-expr} $|$
  \nonterm{call-expr} $|$ \nonterm{set-expr} $|$ \nonterm{binop-expr}
...
\item \ithead{Call expressions.} (1) The called expression must be an
  ...
  nonzero size.
%
\item \ithead{Set expressions.} The right-hand expression must have
  nonzero size.
%
...
Similarly, all stack slots corresponding to saved registers (including
the \kwd{S} register) will contain the values that the corresponding
registers had on exit from the function.
