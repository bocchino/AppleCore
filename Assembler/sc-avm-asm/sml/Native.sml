...
fun getMnemonic substr =
    case (Substring.translate (Char.toString o Char.toUpper) substr) of
      ...
      | "AND" => SOME AND
      | "ASL" => SOME ASL
      | "BLT" => SOME BCC (* Useful alias from SCMASM *)
      | "BCC" => SOME BCC
      | "BGE" => SOME BCS (* Useful alias from SCMASM *)
      | "BCS" => SOME BCS
      | "BEQ" => SOME BEQ
      | "BIT" => SOME BIT
      | "BMI" => SOME BMI
...
fun instBytes (inst,addr,map) = 
    ...
    in
        case inst of
          ...
          | (BIT,expr as ZeroPage _) => operandBytes 0x24 expr
          | (BIT,expr as Absolute _) => operandBytes 0x2C expr
