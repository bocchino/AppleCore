...
3. EDIFY.v2d: Source code and object code for the program "Edify" in
   AppleCore/Examples/Edify.

4. LIB.v2d: The AVM and library source files needed to assemble the
   files on disks 1--3.

5. SHELL.EDITOR.dsk: A shell editor that's a bit nicer than the native
...
1. The v2d format is proprietary to the Virtual II emulator, so the
   first four disk images require Virtual II.  However, you can make
...
3. Booting from the first four disks doesn't work, because they have
   no DOS image.  You can boot from the SHELL.EDITOR disk.  Doing that
   installs the editor, but it can be easily uninstalled.  Or you can
   boot from your own DOS image.
