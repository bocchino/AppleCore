...
\item \ithead{Call expressions.} (1) The called expression must
  ...
  nonzero size. (3) For each argument expression that is (a) an
  identifier corresponding to a variable or function parameter or (b)
  a call expression or (c) a typed expression, either the expression
  and the corresponding parameter of the called function must both
  represent an address, or neither may.
%
...
statement must have nonzero size.  (2) If the left-hand side of a set
statement is an identifier corresponding to a variable or function
parameter, and the right-hand side is (a) an identifier corresponding
to a variable or function parameter or (b) a call expression or (c) a
typed expression, then either the left- and right-hand sides must both
represent an address, or neither may.
...
\bfhead{Binary operation expressions.} \ithead{Shift operations
  (\shl~and \shr).} Evaluate the left-hand side, evaluate the
