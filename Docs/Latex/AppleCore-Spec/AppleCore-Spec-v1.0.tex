...
to the Apple II is currently available.
...
such as symbolic constants, calling addresses as functions, passing
and storing functions as addresses, laying out data tables and jump
tables, etc.  However, AppleCore also provides higher-level
abstractions such as loops, functions, and multi-byte variables,
making it much easier to program than assembly.  The dollar notation
for hexadecimal numbers is supported, so addresses, masks, ASCII
codes, etc. can be expressed in a convenient, Apple II-friendly style.
...
extensibility. For example, unlike Applesoft, which has many built-in
...
The language also incorporates several unusual features that are
perhaps novel or interesting.  First, AppleCore natively supports
integer arithmetic (both signed and unsigned) on fixed-size variables
of up to 255 bytes.  For example, you can declare and initialize
variables \kwd{A} and \kwd{B}, each of size 16 bytes unsigned, and
then the expression \kwd{A+B} also has size 16, and it represents the
16-byte unsigned addition of \kwd{A} and \kwd{B} in the obvious way.
(Variable-sized arithmetic, i.e., where the size is not known at
compile time, is supported via a library.)  Second, a simple type
system keeps track of size and signedness.  The programmer specifies
the size (as a compile-time constant-value expression) and signedness
of each variable and function return value in the program, and the
compiler infers the rest.  Third, \emph{register expressions} allow
AppleCore programs to read and write 6502 registers.  This seems
cleaner than inline assembly (which isn't supported), and it's
essential for low-level routines, e.g., a character printing routine
that loads a value into the accumulator and then calls
\kwd{MON\_COUT}.
...
$$\nonterm{type} ::= \mbox{\nonterm{expr} \opt{\kwd{S}} $|$ \kwd{@}}$$

\noindent
The size expression may not be a binary operation \kwd{=}.
...
\bfhead{Size and signedness associated with type} The compiler
assigns an size and signedness to each type appearing in the program
as follows:
%
\begin{enumerate}
%
\item If the type has the form \nonterm{expr}, then use the rules in
  Section~\ref{sec:code-gen:expressions} to evaluate \nonterm{expr} to
  a numeric constant \nonterm{size}, which must be between 1 and 255
  inclusive.  The type has size \nonterm{size} and is unsigned.
%
\item If the type has the form \nonterm{expr} \kwd{S}, then proceed as
  in case 1, except that the resulting type is signed.
%
\item The type \kwd{@} has size 2 and is unsigned.
%
\end{enumerate}
...
\bfhead{Typed expressions} Evaluate the value subexpression.  Adjust
