...
such as symbolic constants, calling subroutines as functions, passing
...
compile time, is supported by library functions.)  Second, a simple
type system keeps track of size and signedness.  The programmer
...
allow AppleCore programs to read and write 6502 registers.  This
solution is cleaner than inline assembly, and it's essential for
low-level routines, e.g., a character printing routine that loads a
value into the accumulator and then calls \kwd{MON\_COUT}.  Inline
assembly is not supported, but it's easy to link against handwritten
assembly and call it from within AppleCore code.
...
\ithead{(2) Assembly-time constants} If the called expression
