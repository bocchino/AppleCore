...
keyword \kwd{FN}, an optional return type, an identifier representing
...
expression, the size and signedness are computed as follows:
%
\begin{itemize}
%
\item If the expression is a compile-time constant-value expression,
  then the size and signedness come from the value produced by
  evaluating the expression as specified in
  Section~\ref{sec:code-gen:expressions}, assuming that the operands
  to any binary expression have the maximum allowed size of 255.  The
  size is the minimum number of bytes required to represent the value.
  The expression is signed if the value is less than zero, otherwise
  unsigned.
%
\item Otherwise, the size is 2 and the signedness is unsigned.
%
\end{itemize}
%
For an expression that is not a constant-value expression, the size
and signedness are computed as follows:
%
\begin{itemize}
%
