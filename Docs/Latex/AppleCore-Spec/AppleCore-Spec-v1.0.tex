...
initialize variables \kwd{A} and \kwd{B}, each of size 16 bytes
...
\kwd{MON\_COUT}, and register expressions provide a much cleaner
...
\begin{enumerate}
%
\item \ithead{Separation of tokens} When a whitespace character or end
...
\kwd{SET}, a term, an equals sign, a right-hand-side expression, and a
terminating semicolon:
%
...
example, \kwd{@F()} is parsed as \kwd{@(F())} and not \kwd{(@F)()}.
...
\bfhead{Indexed expressions} An indexed expression consists of a base
term, an offset expression, and a type:
%
$$\nonterm{indexed-expr} ::= \mbox{\nonterm{term} \kwd{[}
    \nonterm{expr} \kwd{,} \nonterm{type} \kwd{]}}$$

\bfhead{Call expressions} A call expression consists of a term
...
\bfhead{Typed expressions} A typed expression is a term with an
explicit type:
%
...
\begin{itemize}
%
\item \ithead{Integer constants} An integer constant represents an
  address if its size is 1 or 2.
%
\item \ithead{Identifiers} An identifier represents an address if its
  ...
  or (2) is a data declaration; or (3) is a function declaration; or
  (4) is a variable or function parameter declaration whose type is
  \kwd{@}.
%
...
\bfhead{Valid address assignments} If the left-hand side of an
