...
abstraction as C, but without C's features for structs) that's both
useful and fun to program with.  First, it's a compiled language, so
it should run faster than Applesoft.  Second, it's modular and
...
with assembly language routines: you can say things like ``load `A'
into the accumulator and call \$FDED'' that are impossible to say
directly in Applesoft.  While you could use the \kwd{USR} function or
a \kwd{CALL}, in either case you'd need to write some glue code in
...
\kwd{SET}, an lvalue expression, an equals sign, a right-hand-side
...
An expression is an lvalue expression, a numeric constant, a call
expression, a binary operation expression, a unary operation
expression, or a parentheses expression.
%
$$
%
\nonterm{expr} ::= \mbox{\nonterm{lvalue-expr} $|$
...
side of a set statement.  It is an identifier, an indexed expression,
...
\item In any sequence \nonterm{expr-1} \nonterm{binop-1}
  ...
  of precedence, from highest to lowest: (1) \kwd{<<} and \kwd{>>};
...
\bfhead{Size and signedness of expressions.} Every expression
...
\item \ithead{Identifiers.} The size and signedness of an identifier
  ...
  signedness is signed if \kwd{S} appears after the size, otherwise
...
\item \ithead{Call expressions.} If the called expression (i.e., the
  ...
  (1) If a \nonterm{size} appears after the keyword \kwd{FN} in the
  definition, then that is the return size; otherwise the return size
  is 0.  (2) If \kwd{S} appears after \nonterm{size} then the
...
\bfhead{Size and signedness requirements for expressions.} The
...
\begin{itemize}
%
\item \ithead{Indexed expressions.} Both the base expression (before
  the first bracket) and the index expression (immediately after the
  first bracket) must have size 1 or 2.
%
\item \ithead{Call expressions.} (1) The called expression must be an
...
Section~\ref{sec:code-gen:global-decls}.  In include mode, the source
program may not contain any include directives, and it is a
compile-time error for one to appear.
...
Notice that the right-hand side of the set statement is evaluated
first; this is for implementation efficiency reasons.  In particular,
the expression \kwd{SET X[0,1]=X+1} is evaluated by \emph{first}
adding 1 to \kwd{X} and \emph{then} computing the destination address,
so the effect is to store the original value of \kwd{X} plus 1 at the
location given by the original value of \kwd{X} plus 1.
